    ...
    function convertOCShapeToBabylonMesh(shape, name = "OCShape") {
      ...
      try {
        ...
        while (faceExplorer.More()) {
          ...
          // CRITICAL: Use 3-parameter signature with Poly_MeshPurpose_NONE (0)
          // This is the CORRECT API signature verified by expert
          const triangulation = ocInstance.BRep_Tool.Triangulation(face, location, 0); // 0 = Poly_MeshPurpose_NONE
    ...
    // Convert user-friendly OpenCascade code to proper OpenCascade.js syntax
        function convertSimplifiedCode(code) {
            ...
            const helperFunctions = `
                // VERIFIED Helper functions for OpenCascade.js API
                // All patterns confirmed by expert and tested
                const occ = {
                    // CORRECT: Box requires origin point + 3 dimensions (4 params total)
                    ...
                    union: function(shape1, shape2) {
                        if (Array.isArray(shape2)) {
                            ...
                            // Single shape union
                            // CORRECT: BRepAlgoAPI_Fuse_3 requires 3 params (shape1, shape2, progress)
                    ...
                    // VERIFIED: Create wire from edges
                    createWire: function(edges) {
                        ...
                        if (edges.length === 1) {
                            // CORRECT: Use single-edge constructor
                            return new oc.BRepBuilderAPI_MakeWire_2(edges[0]).Wire();
                        } else {
                            // CORRECT: Use BRepBuilderAPI_MakeWire_1() then Add_2 for each edge
                            const wireBuilder = new oc.BRepBuilderAPI_MakeWire_1();
                            for (let edge of edges) {
                                wireBuilder.Add_2(edge); // Add_2 is the correct method for edges
                            }
                            return wireBuilder.Wire(); // Already returns proper TopoDS_Wire
                        }
                    },
                    
                    // VERIFIED: Polygon wire creation from points
                    // Uses optimized constructors based on point count
